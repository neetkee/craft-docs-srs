...
Each Craft collection becomes a deck. Each item in the collection is one flashcard. A line separator (horizontal rule) splits the item content into front and back — everything before the separator is the front of the card, everything after is the back. The back can be empty.  When you add a collection as a deck, the app automatically adds an SRS text property to the collection schema if it doesn't exist. Review metadata is stored in that property.
...
## Demo
[![asciicast](https://asciinema.org/a/892215.svg)](https://asciinema.org/a/892215)